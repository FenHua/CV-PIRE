...
# paris buildings
with open("./src/gnd_paris6k.pkl", "rb") as fp:   # Unpickling
    cfg = pickle.load(fp)
...
with open("./src/pa_noresize_res101_2048.pkl", "rb") as fp:   # Unpickling
...
try:
    os.remove(location + 'all_iter.csv')
except: 
    pass

treshFile = open(location + 'all_iter.csv','w+')
treshFile.write('Iter,mAp' + '\n')

#Change this for a different database
treshFile.write('0,74.419869991' + '\n')

for it in tqdm(range(args.start_iter,args.max_iter + args.step_iter, args.step_iter)):
    mAP = []
    query_imgs = []
    MRR = []
    map_tt = []
    
    adress = location + 'T' + str(it)+ '/'
    filename = 'map_score_img_output_T' + str(it) + '.txt'
    
    try:
        os.remove(adress+filename)
    except:
        pass
    
    file = open(adress+filename, 'w+')
    ...
    print("Iter" + " " + str(it) + ": " + str(r1))
    
    for i in range(len(r2)):
        file.write(query_imgs[i] + '\t\t : ' + str(r2[i]) + '\n')

    file.write('\n')
    file.write('------------------------------\n')
    file.write('average MaP \t\t : ' + str(r1) + '\n')
    file.close()

    #this is in percentages %
    treshFile.write(str(it) + ',' + str(100*r1) + '\n')

    print("Iter" + " " + str(it) + " has mAP: " + str(r1))

treshFile.close()
print("Wrote all the mAP scores to files")
